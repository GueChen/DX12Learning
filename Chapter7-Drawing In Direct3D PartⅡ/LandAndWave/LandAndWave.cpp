...
bool LandAndWavesApp::Initialize()
{
	...
	// Excute the initialization commands.
...
void LandAndWavesApp::OnResize()
{
	...
	// window resized, so update the aspect ratio and recompute projection matrix
...
void LandAndWavesApp::Update(const GameTimer& gt)
{
	...
	// Has the GPU finished processing the comands of the current frame resource?
	// If not, wait until the GPU has completed commands up to this face point.
...
void LandAndWavesApp::Draw(const GameTimer& gt)
{
	...
	// A command list can be reset after it has been added to the command queue via ExcuteCommandList.
	...
	{
	D3D12_RESOURCE_BARRIER Pre2RTBarrier = CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(), D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET);
	mCommandList->ResourceBarrier(1, &Pre2RTBarrier);
	}
	...
	D3D12_CPU_DESCRIPTOR_HANDLE CurrDSV = DepthStencilView();
	mCommandList->OMSetRenderTargets(1, &CurrBBV, true, &CurrDSV);
	}
	...
	// Indicate a state transition on the resource usage.
	{
	D3D12_RESOURCE_BARRIER RT2PreBarrier = CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PRESENT);
	mCommandList->ResourceBarrier(1, &RT2PreBarrier);
	}
...
void LandAndWavesApp::UpdateObjectCBs(const GameTimer& gt)
{
	...
	for (auto& e : mAllRitems)
	{
		...
		// This needs to be tracked per framer resource.
...
void LandAndWavesApp::UpdateWaves(const GameTimer& gt)
{
	...
	auto currWaveVB = mCurrFrameResource->WavesVB.get();
