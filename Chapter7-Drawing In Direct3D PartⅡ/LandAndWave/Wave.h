...
public:
	...
	// Returns the solution at i-th grid point.
	const DirectX::XMFLOAT3& Position(int i) const { return mCurrSolution[i]; }

	// Returns the solution normal at i-th grid point.
	const DirectX::XMFLOAT3& Normal(int i) const { return mNormals[i]; }

	// Returns the unit tagent vector at the i-th grid point in the local x-axis direction.
	const DirectX::XMFLOAT3& TagentX(int i) const { return mTangentX[i]; }
