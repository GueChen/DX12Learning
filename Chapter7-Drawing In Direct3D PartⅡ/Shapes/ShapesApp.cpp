...
#define WIDEN_HELPER(x) L ## x
#define WIDEN(x) WIDEN_HELPER(x)
...
void ShapesApp::Draw(const GameTimer& gt)
{
	...
	{
		D3D12_RESOURCE_BARRIER RTBarrier = CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(),
			D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET);
	mCommandList->ResourceBarrier(1, &RTBarrier);
	}
	...
	// Indicate a state transition on the resource usage.
	{
	D3D12_RESOURCE_BARRIER PresentBarrier = CD3DX12_RESOURCE_BARRIER::Transition(CurrentBackBuffer(),
...
{
	mShaders["standardVS"] = d3dUtil::LoadBinary(WIDEN(SHADER_VS_BIN));
	//d3dUtil::CompileShader(L"Shaders\\color.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["opaquePS"] = d3dUtil::LoadBinary(WIDEN(SHADER_PS_BIN));
...
void ShapesApp::BuildPSOs()
{
	...
	//
	memset(&opaquePsoDesc, 0, sizeof(D3D12_GRAPHICS_PIPELINE_STATE_DESC));
